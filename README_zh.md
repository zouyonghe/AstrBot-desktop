...
桌面端默认只监听 `127.0.0.1:6185`，仅允许本机访问。如果确认需要在同一局域网内访问，可以在桌面端配置文件中把 dashboard host 改为 `0.0.0.0`。
...
```text
C:\Users\<用户名>\.astrbot\data\config\desktop.json
```
...
如需恢复默认本机访问，可删除 `dashboard` 配置，或把 `host` 改回 `127.0.0.1` 后重启应用。

环境变量仍可作为高级覆盖项，且优先级高于 `desktop.json`：
